...
describe("POST /registera", function () {
  it("Should respond with error 400 if not all fields are filled", (done) => {
    backend.post("/registrera", {}).end(function (err, res) {
      if (err) throw err;
    ...
    backend
      ...
      .end(function (err, res) {
        ...
        expect(res.body.password).to.equal(
          "Lösenordet måste vara minst 7 karaktärer"
        );
        done();
      });
  });
...
describe("POST /loggain", function () {
  it("Should respond with error 400 if not all fields are filled", (done) => {
    backend.post("/loggain", {}).end(function (err, res) {
      if (err) throw err;
      expect(res.status).to.equal(400);
      done();
    });
  ...
  it("Should respond with error 404 if the email is not found", (done) => {
    backend
      ...
      .end(function (err, res) {
        ...
        expect(res.body.emailnotfound).to.equal(
          "Angiven mailadress och lösenord matchar inte. Försök igen."
        );
        done();
      });
  });
  it("Should respond with error 400 if the email or the password is incorrect", (done) => {
    backend
      ...
      .end(function (err, res) {
        if (err) throw err;
        expect(res.status).to.equal(400);
        expect(res.body.passwordincorrect).to.equal(
          "Angiven mailadress och lösenord matchar inte. Försök igen."
        );
        done();
      });
